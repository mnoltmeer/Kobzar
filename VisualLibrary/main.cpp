...
//лямбда, що модифікує параметр на основі хаотичного коефіцієнту
	   ...
	   res->AddEllipse(rect); //додаємо основний еліпс, в якому буде текст

//малюємо бульбашки хмарки
...
//малюємо хвостик
	   ...
	   float dx = tail_point.X - tail_base.X,
			 dy = tail_point.Y - tail_base.Y; //знаходимо довжину вектора між основою та вершиною хвостика

//спускаємось по вектору до точок, де будуть бульбашки хвостика
	   ...
	   for (const auto c : FItems)
		 {
		   ...
		   if (!hdwp)
			 break; //помилка, не вистачило пам’яті або handle зіпсований
		 }

//застосовуємо зміни одним пакетом
	   if (hdwp)
